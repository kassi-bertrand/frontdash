...
/**
 * =============================================================================
 * TODO: CONNECT RESTAURANT REGISTRATION TO BACKEND
 * =============================================================================
 *
 * CURRENT STATE: handleSubmit (lines 413-426) uses setTimeout fake submission
 * TARGET STATE: POST to Express backend for real registration
 *
 * BACKEND ENDPOINT:
 *   URL: ${process.env.NEXT_PUBLIC_API_URL}/api/restaurants/register
 *   Method: POST
 ...
 *     "street_address": string,    // Combine buildingNumber + streetName
 *     "city": string,
 *     "state": string,
 *     "zip_code": string,          // NOTE: Form doesn't have zip yet - ADD IT
 *     "phone_number": string,      // First phone from array
 ...
 *   409: { "error": "Restaurant name already exists" }
 *
 * IMPLEMENTATION (replace handleSubmit lines 413-426):
 ...
 *         street_address: `${form.buildingNumber} ${form.streetName}`,
 *         city: form.city,
 *         state: form.state,
 *         zip_code: form.zipCode,  // ADD zip_code to form state
 *         phone_number: sanitizePhone(form.phones[0]),
 *         restaurant_image_url: null,  // TODO: Handle image upload
 *       });
 *       setCredentials(result.credentials);
 *       setSubmitted(true);
 *       setApprovalStatus('pending');  // Backend returns as PENDING
 *     } catch (error) {
 ...
 *
 * ADDITIONAL TODOS:
 *   1. Add zip_code field to form (line ~112 RestaurantDraft type)
 *   2. Add zip code input field in step 0 address section
 *   3. Handle image upload (upload to S3/storage first, get URL)
 *   4. Operating hours: POST /api/restaurants/:id/hours after registration
 *   5. Menu items: POST /api/restaurants/:id/menu after registration
 *   6. Remove "Simulate admin approval" button - real flow requires admin
 *
 * NOTES:
 *   - Registration creates restaurant with status='PENDING'
 *   - Credentials are auto-generated by backend
