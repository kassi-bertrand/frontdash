...
#
# Reads db-config.txt to allow EC2 → RDS network access
...
#   5. npm install + npm run build (TypeScript → JavaScript)
#   6. Start the API with PM2 (process manager)
# --------------------------------------------------
echo "Step 4/4: Deploying API to EC2..."

if [ ! -f "deploy-api.sh" ]; then
    echo "❌ deploy-api.sh not found. This is generated by setup-ec2.sh"
    exit 1
fi
